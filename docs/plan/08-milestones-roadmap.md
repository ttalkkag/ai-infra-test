...
> 확정 범위: 엔진 코어 + Template/Sandbox + Local Runner + Web UI 전체(로컬). 클라우드는 후속. ([[00-overview]] D6/D11)
...
```
M0 부트스트랩 ─ M1 데이터·안전 코어 ─ M2 엔진/매칭 ─ M3 Sandbox MVP ─ M4 Local Runner MVP ─┊─ M5 Cloud 설계 ─ M6 배포
                                                                  (로컬 범위 끝)   (후속)
```

의존: M1→M2→M3→M4 순차(각 단계가 앞 산출물에 의존). M0는 전 단계 토대. M5/M6은 M4 검증 후.
...
- **검증**: CI green, 빈 엔드포인트 계약 테스트 통과, 목 LLM으로 결정적 실행 확인
...
- **위험·완화**: 도구 미설치/버전 차 → 버전 고정·설치 점검·CI 캐시; 생성기 자체 병목 오판 → 생성기 신호 분리 테스트; 로컬 자원 한계 → 목 서버·부하 상한을 보수적으로
...
- **목표**: Terraform `plan→정책/비용→승인→apply` 게이트와 ephemeral runner·teardown을 설계하고 일부 구현. 실제 고부하·실서비스 대상은 여전히 금지.
- **산출물**: Terraform module skeleton 계획, plan JSON policy checklist, approval workflow(plan_hash↔approval), canary→full 승격 규칙, teardown/TTL 회수, OIDC 단명 토큰·비용 3중 백스톱 설계
...
- **위험·완화**: cleanup 실패 → TTL sweeper+cost alarm; 비용 폭증 → max cost budget+auto abort; **Azure 경로 시 지원 엔진/region 재확인(`2026-04-01` dataplane API는 GA 확정), AWS DLT MCP=read-only/도입 v4.1.0(2026-05-13) 반영**([[10-open-questions-reverify]])

## M6 · (후속) 사내/클라우드 배포·실대상 연동

- **목표**: 사내 서버 또는 클라우드 배포, 실제 dev/staging 대상 연동(승인·env allowlist·kill switch 선적용).
- **산출물**: 배포 IaC, 인증/권한, 관측 연동(Grafana/Prometheus), 운영 런북
- **검증**: 실대상은 read-only smoke부터, on-call 승인·시간창·공지·rollback 없으면 실행 불가
