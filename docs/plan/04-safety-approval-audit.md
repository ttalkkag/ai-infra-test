...
| A8 | **목 서버 대상** canary 이하 저부하 실행(로컬, 상한 내) | 로컬 결정성·비용0([[00-overview]] D8), 단 full run은 canary 통과 전제([[00-overview]] §6) |
...
clamp(slots, template, env, target) -> ClampResult:
    ...
    # (2) target_whitelist — 화이트리스트 외 대상은 거부
    if template.target_whitelist and target not in template.target_whitelist:
        return REJECT(reason="target_not_whitelisted")   # 로컬: 목 서버만 화이트리스트
    ...
    for field in [rps, vu, duration, cost]:
        ...
        if slots[field] > cap:
            ...
            if requester_is_nonexpert:        # 비전문가 = 우회 불가 → 상향은 승인으로만
                effective_slots[field] = cap   # 하향 clamp 후
            else:
                effective_slots[field] = slots[field]  # 전문가도 상향 자체는 승인 필요
        else:
            effective_slots[field] = slots[field]

    # (4) 판정
    if violations and requested_above_cap:
...
| 불변식 | 강제 방법 |
|---|---|
| 비전문가는 상한을 **상향할 수 없다** | `requester_is_nonexpert`면 초과값을 cap으로 하향(CLAMPED) 또는 차단. 상향 경로는 P5 승인 게이트만 |
...
approval = ApprovalRequest(
    ...
    status="pending")
persist_tamper_proof(approval, plan_hash)        # append-only, 사후 변조 불가
...
- append-only: 승인 레코드와 plan_hash는 `audit.py`의 append-only 스토어(§6)에 함께 기록, UPDATE/DELETE 미허용.
...
| `clamp` | 경계값 | rps = cap / cap+1 / cap-1 | OK / NEEDS_APPROVAL or CLAMPED / OK |
