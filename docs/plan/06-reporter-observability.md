...
6. **warmup 구간 분리.** cold-start/워밍업 표본이 최종 집계에 섞이면 해석 불가(방법론 함정 8: cache cold/warm 혼합). 시나리오는 warmup phase를 별도 단계로 태깅하고 collector/interpreter는 warmup 표본을 SLO 집계에서 제외한다. [[05-runners-and-mock-target]] §3.2의 `delayAbortEval`은 **abort 평가만 유예**할 뿐 집계 제외가 아니므로 이를 대체하지 못한다. 도구 산출물이 warmup 분리를 지원하지 않으면 `limitations`에 "warmup 미제외"를 자동 표기한다.
...
| **쿼터/플랫폼 한계** | `error.429_rate`, `error.resource_exhausted_rate` | 일반 4xx/5xx와 **분리 집계**. 초과는 SUT 성능 한계가 아니라 **플랫폼 쿼터/램프 한계**로 분류(별도 finding) | BaaS/Cloud Run 대상이면 통과 게이트에 쿼터 가드 필수(방법론 §12.2·§13.1). 백오프 재시도 유도 자체가 쿼터 병목 증거 — retry storm·비용 증폭 주의. 로컬 목 서버에선 미발생이나 Firebase 템플릿 2종이 초기 5종에 포함되므로([[03-engine-template-matching]] §7) 축·파서는 M4에 준비 |
...
초기 confidence 조정 상수는 다음으로 고정한다. 수치는 리포트 표현 강도를 낮추기 위한 보수값이며, 판정(outcome)을 뒤집는 용도가 아니다.

| 조건 | confidence 처리 |
|---|---|
| 생성기 포화 | `min(confidence, 0.50)` + outcome=`inconclusive` 후보 |
| 필수 신호 누락 | 누락 신호마다 `confidence *= 0.70`, 하한 `0.30` |
| frozen/synthetic 입력 | `min(confidence, 0.60)` |
| client-side percentile만 존재 | `confidence *= 0.85` |
| 짧은 테스트 윈도 burn-rate 단독 근거 | `confidence *= 0.80`, limitations에 윈도 한계 표기 |
...
def interpret(obs: ObservationBundle, plan: TestPlanSpec,
    ...
    if current_run.status == "aborted":
        return aborted_spec(obs, reason=current_run.abort_reason, evidence=refs)
...
- 버킷 경계 설계 주의(방법론 §8-5: histogram 오차는 rank가 아니라 **값(버킷 경계) 기준**, summary가 rank(φ) 기준)는 cloud OTel 단계에서 관심 percentile 주변을 촘촘히. 로컬에는 해당 없음.
...
초기 회귀 임계는 관측 잡음을 피하기 위해 절대·상대 조건을 함께 둔다.

| 비교 키 | 회귀 임계 |
|---|---|
| `latency.p95` | `delta_pct >= 10%` 그리고 `delta_abs >= 20ms` |
| `latency.p99` | `delta_pct >= 15%` 그리고 `delta_abs >= 30ms` |
| `error.*_rate` | `delta_abs >= 0.5pp` 또는 상대 25% 악화 |
| burn rate | `delta_pct >= 25%` 또는 임계 초과 전환 |
| dropped_iterations | 0 → 양수 전환 또는 상대 25% 악화 |
...
| 포맷 | 용도 | 비고 |
|---|---|---|
| **웹 뷰**(JSON → React) | 1차 소비. 경영/기술 탭 분리, evidence_ref 클릭 시 원천 artifact 드릴다운 | [[07-web-ui-api]]가 렌더. 원시 artifact(stdout·생성 시나리오)도 드릴다운 노출 전 로그 채널과 **동일한 redaction**(대상 URL 내 자격증명·인증 헤더 마스킹) 적용 — 참조-only 원칙(§2.2)을 드릴다운이 우회하지 않게 한다 |
...
- **cross-cloud(X0, 후속):** runner Provider와 target Provider의 시계·샘플링·집계 지연이 다를 수 있다([[12-execution-topology-matrix]]). 리포트는 `runner_provider`/`target_provider`/`network_path`를 표시하고, egress·source IP/load zone·metric delay를 `limitations`에 포함한다.
...
| **aborted** | `RunRecord.status=aborted` | `aborted` | kill switch 경로 |
