...
>
> 범위 경계: 이번 로컬 범위는 k6/Artillery 바이너리를 로컬에서 돌려 **번들 목 서버**를 친다 ([[00-overview]] D6·D8). 클라우드 분산 러너(Grafana Cloud k6 / AWS DLT / Azure LT)는 **동일 `RunnerAdapter` 인터페이스 뒤의 후속 확장**으로 §8에만 설계한다 ([[00-overview]] D11·[[08-milestones-roadmap]] M5+).
>
...
class ComposedArtifact:
    ...
    content_hash: str         # plan_hash 검증·골든 테스트 동일성 키
```

> `content_hash`는 승인 시점의 `plan_hash`와 묶여 변조 방지에 쓰인다([[04-safety-approval-audit]]). 같은 슬롯 입력 → 같은 `content_hash`(결정성)가 골든 파일 테스트의 전제(§9).
...
def execute(action: ToolAction, plan: TestPlanSpec) -> RunRecord:
    ...
    _verify_plan_hash(action, composed)           # content_hash ↔ 승인된 plan_hash
...
| plan_hash 결합 | 승인 시점 `plan_hash`와 `compose` 산출 `content_hash`가 일치해야 실행 — 불일치=변조로 거부 |
...
| 큐/포화 | `queue.depth_max`, `queue.drain_rate` | 단순 M/M/1 유사 큐: 적체 시 지연 증가→포화 시 503 |
...
- k6/Artillery **바이너리는 로컬에 설치**되어 있다고 가정(설치 자동화는 비범위; 버전은 고정·`k6 version`/`artillery -V`로 기록 — docs/research/01 정정·docs/research/04 §9 재검증).
...
동일 `RunnerAdapter`(§1) 뒤에 클라우드 러너를 **인터페이스만** 끼운다. `controller.py`는 변경 없이 `tool`/`topology`로 어댑터를 디스패치([[02-data-model]] `ProvisionSpec.topology`). 실제 구현·실행은 [[08-milestones-roadmap]] M5+, 본 로컬 범위 제외([[00-overview]] D11).
...
| `runners/aws_dlt_adapter.py` | Taurus on ECS/Fargate, 시나리오 REST/CLI 정의 | **AWS DLT MCP는 read-only**(7 tools), 도입 **v4.1.0(2026-05-13)**(MCP+CLI, 공식 CHANGELOG) — MCP만으론 실행 불가([[00-overview]] §5). 실행은 REST/CLI 경로로 설계 |
| `runners/azure_lt_adapter.py` | JMeter/Locust 엔진(.jmx 업로드) | dataplane REST API **`2026-04-01` GA 확정**(`azure-rest-api-specs` `stable/`·default moniker) → Azure 경로 채택 시 **지원 엔진/region**만 재확인([[00-overview]] §5·docs/research/04 §9) |

확장 시 불변식 보존: **실행 권한은 여전히 controller만**, ToolAction typed-only, canary→full 승격, 승인+plan_hash, kill switch — 클라우드라고 완화하지 않는다. 차이는 `submit/collect`의 백엔드(로컬 프로세스 → 클라우드 API)뿐이고, `validate/compose/dry_run`은 로컬과 공유한다.
...
|---|---|
| `ComposedArtifact.content_hash` | `ApprovalRequest`/`plan_hash`(변조 방지) |
