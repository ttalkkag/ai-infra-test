...
- **직렬화 규칙(인젝션 방어)**: 이후 절의 `{{PLACEHOLDER}}` 골격은 이해를 돕는 표기일 뿐, 구현의 compose는 **문자열 치환으로 실행 산출물(YAML/JS)을 만들지 않는다**. Artillery config는 검증된 dict를 `yaml.safe_dump`로 직렬화해 생성하고(따옴표·개행으로 문맥 이탈 불가), k6 JS의 문자열 슬롯은 언어별 이스케이프를 강제한다. 본문 보간 슬롯은 숫자/enum 위주로 제한한다.
- **인증값은 secret-ref만**: 슬롯에는 시크릿 "값"이 아니라 참조(환경 키 이름 등)만 담는다. 값은 controller가 실행 시점에만 해석해 러너 프로세스 env로 주입하며, `ToolAction.args`·audit log·`/plan` 응답·리포트 직렬화에는 값이 실리지 않는다(redaction — [[04-safety-approval-audit]] §10 audit은 ref만 기록).
...
def execute(action: ToolAction, plan: TestPlanSpec) -> RunRecord:
    ...
    require_approval(action)                      # §5.2 — policy.classify() 재도출 기반(fail-closed).
                                                  #   자기신고 requires_approval 플래그는 신뢰 입력이 아님
...
| 검사 | 규칙 |
|---|---|
| 정책 재도출(fail-closed) | 컨트롤러는 `action.requires_approval`(상류가 채운 자기신고 값)을 신뢰하지 않고, 실행 직전 `safety.policy.classify()`([[04-safety-approval-audit]] §2)를 **독립 재실행**한다. 재도출 결과가 DENY면 무조건 abort, REQUIRE_APPROVAL이면 아래 승인 검사로 진행 — `requires_approval=false`는 검사 생략 사유가 아니다 |
| 승인 존재 | 재도출 결과가 REQUIRE_APPROVAL이면 `approval_id` 필수, `ApprovalRequest.status="approved"`만 통과 |
...
| `GET /metrics` | REST(관측) | Prometheus 텍스트 노출(`process_cpu_seconds` 등) — reporter collector의 `target.cpu` 등 saturation 대리값 원천([[06-reporter-observability]] §2.1·§2.2) |

- WebSocket 경로는 **부하 단위=연결**(docs/research/04 §11.4)을 모사: 연결 수립 지연·세션 지속·메시지 왕복(ping)으로 `ws_*` 대리 지표를 어댑터가 관측 가능하게 한다.
- `__control/*`·`/metrics`는 로컬 셋업/관측 전용이며 실행 평면 ToolAction 경로와 분리(러너는 부하 엔드포인트만 친다).
...
| 데이터 파일 경계 | 슬롯이 참조하는 CSV/JSON 데이터 파일 경로는 realpath 정규화 후 지정 데이터 디렉토리(`runs/<run_id>/` 또는 번들 데이터 디렉토리) prefix 안에서만 허용 — 경로 순회(`../`)·시스템 경로 참조는 `validate`에서 거부 |
...
### 7.3 dry-run 검증(승인 전) → 승인 → 실제 실행 — 플립 금지

```
draft ToolAction(dry_run=true)  ──▶  compose + dry_run(정적/무부하, A5 자동 허용)  ──▶  계획 검토
        │  검증 통과 → 실행 번들 확정: 별도 ToolAction(dry_run=false) + plan_hash 계산([[02-data-model]] §4)
        ▼
사람 승인(approval_id + plan_hash 결속)
        ▼
승인된 ToolAction(dry_run=false)  ──▶  submit → canary → full_run → collect
```

- 승인 전에 실행되는 것은 `dry_run=true` 정적 검증(네트워크 0)뿐이라, 승인 전에는 어떤 실제 부하도 발생하지 않는다([[00-overview]] §6, [[04-safety-approval-audit]]).
- **`dry_run` 플립 금지**: 승인된 액션의 `dry_run` 값을 뒤집는 경로는 없다. dry-run 검증과 실제 실행은 **별개 ToolAction**이고, 승인 번들에는 처음부터 `dry_run=false` 실행 액션을 담는다. `dry_run`은 `plan_hash` 입력에 포함되므로([[02-data-model]] §4) 승인 후 값이 바뀌면 해시 불일치로 abort된다. 실행 모드를 바꾸려면 새 번들로 재승인한다.
...
| **정적 검증 단위** | `validate`/`dry_run` | 잘못된 슬롯(타입·필수 누락·safe_limits 초과·셸 메타문자·YAML/JS 문맥 이탈 문자(따옴표/개행)·데이터 파일 경로 경계 위반) → 거부 확인. 네트워크 0 |
